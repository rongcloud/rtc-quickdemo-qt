...
RCMeeting::RCMeeting(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::meeting)
{
    ui->setupUi(this);
...
bool RCMeeting::event(QEvent *event)
{
...
//订阅的状态回调
void RCMeeting::subscribeCb(enum_rcrtc_code code, HANDLE context)
{
    if (code != 0){
...
//订阅指定的用户
void RCMeeting::subscribe(QString userId)
{
    //创建视频流渲染相关的资源
...
//订阅房间内所有用户
void RCMeeting::subscribeAllUsers()
{
...
//取消订阅房间内所有用户
void RCMeeting::unsubscribeAllUsers()
{
...
//取消订阅的状态回调
void RCMeeting::unsubscribeCb(enum_rcrtc_code code, HANDLE context)
{
    if (code != 0){
...
//取消订阅指定的用户
void RCMeeting::unsubscribe(QString userId)
{
...
//建立连接的状态回调
void RCMeeting::connectCb(HANDLE context, rc_connection_status code)
{
    RCMeeting *model = (RCMeeting *)context;
    QCoreApplication::postEvent(model, new RtcEvent(EventConnect, ""));
}

//建立连接
void RCMeeting::connect()
{
    if (!_isConnected){
...
//房间状态回调
void RCMeeting::roomCb(const char *room_id, const char *remote_user, enum_rcrtc_room_event code, void *event_param, HANDLE context)
{
    RCMeeting *model = (RCMeeting *)context;
...
//加入房间的状态回调
void RCMeeting::joinRoomCb(const char *room_id, enum_rcrtc_code code, HANDLE context)
{
    RCMeeting *model = (RCMeeting *)context;
...
//离开房间的状态回调
void RCMeeting::leaveRoomCb(enum enum_rcrtc_code code, HANDLE context)
{
    RCMeeting *model = (RCMeeting *)context;
...
//发布流的状态回调
void RCMeeting::publishCb(enum_rcrtc_code code, HANDLE context)
{
...
//取消发布流的状态回调
void RCMeeting::unpublishCb(enum_rcrtc_code code, HANDLE context)
{
 ...
 * @brief 枚举设备摄像头输入设备
 */
void RCMeeting::ListVideoCapture() {
    //@ 创建摄像头枚举句柄
 ...
 * @brief 打开本地本地摄像头设备
 */
void RCMeeting::OpenVideoCaptureDevice(int index) {
 ...
 * @brief 关闭本地本地摄像头设备
 */
void RCMeeting::CloseVideoCaptureDevice() {
    if (_isStartVideoDevice){
 ...
 * @TODO():
 */
void RCMeeting::ListAudioDevice() {
    int ret = 0;
 ...
 * @brief 打开本地麦克风和扬声器设备
 */
void RCMeeting::OpenMicSpkDevice() {
 ...
 * @brief 关闭本地麦克风和扬声器设备
 */
void RCMeeting::CloseMicSpkDevice() {
    //@ 禁用麦克风(mic)扬声器(spk)设备
    rcrtc_disable_audio();
}

void RCMeeting::dispost(){
    disconnect();
}
